...
While the imported Shopify order contains information about VAT, the VAT gets recalculated when you create sales document. That's why it's important that the VAT/Tax settings are correct in [!INCLUDE[prod_short](../includes/prod_short.md)].
...
- Address-dependent VAT rates. Use the **VAT area priority** field together with **Customer Templates** table to overwrite standard logic that fills in **VAT Area Code** in the sales document. The **VAT area priority** field specifies priority from where the function should take information about country/region and county/province. Then the corresponding record in the Shopify customer templates is found and **VAT Area Code**, **VAT Liable**, and **VAT Bus. Posting Group** is used when a sales document is created.
