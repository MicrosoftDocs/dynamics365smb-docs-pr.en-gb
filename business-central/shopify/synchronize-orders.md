...
In the **VAT area source** field, you can set the priority on how to select VAT area code or VAT business posting group based on address. This step is relevant for countries with VAT. Learn more at [VAT remarks](synchronize-orders.md#tax-remarks).
...
### <a name="tax-remarks"></a>VAT remarks

The imported Shopify order contains information about VAT, but the VAT get recalculated when you create the sales document so it's important that the VAT settings are correct in [!INCLUDE[prod_short](../includes/prod_short.md)].

- Multiple product VAT/tax rates. For example, some product categories are liable for reduced VAT rates. Those items must exist in [!INCLUDE[prod_short](../includes/prod_short.md)] and be mapped to Shopify products. Otherwise, with automatic creation of missing items, the VAT product posting group is used.

- Address-dependent VAT rates. Use the **VAT area priority** field together with the **Customer Templates** table to overwrite standard logic that fills in the **VAT Area Code** in the sales document. The **VAT area priority** field specifies the priority used to determine the information about the country/region and county/province. Then the corresponding record in the Shopify customer templates is found and the **VAT Area Code**, **VAT Liable**, and **VAT Bus. Posting Group** are used when a sales document is created.

- Price including VAT. The **Prices including VAT**/**Prices including VAT** field in the created sales document doesn't depend on the customer, but on the **Customer Template** from the **Shopify Shop Card** page or Customer template per country.
...
|Edit|Impact|
|------|-----------|
|Change the location to another location, mapped to the Shopify Locations. Post shipment. | After synchronising the fulfilment, location will be updated in Shopify. |
