...
ms.locfileid: "9361617"
---
# <a name="synchronize-items-and-inventory"></a>Synchronise Items and Inventory

The **Items** in [!INCLUDE[prod_short](../includes/prod_short.md)] are equivalent to the *products* in Shopify, and include physical goods, digital downloads, services, and gift cards that you may sell. There are two main reasons to synchronise the items:
...
1. Choose the search ![Lightbulb that opens the Tell Me feature.](../media/ui-search/search_small.png "Tell me what you want to do") icon and enter **Shopify Shop**. Open the shop for which you want to configure item synchronisation.
...
|**SKU Field Separator**|Use this with **SKU Mapping** set to the **Item. No + Variant (synchronise-items.md#effect-of- shopify-product-skus-and-barcodes-on-mapping-and-creating-items-and-variants-in-business-central)Code** option.<br> Define a separator to be used to split the SKU.<br>So, if in Shopify you create the variant with the SKU '1000/001', you'd type '/' in the **SKU Field Separator** field to make the item number in [!INCLUDE[prod_short](../includes/prod_short.md)] '1000' and the item variant code '001'.|
...
Prices for synchronised items can be exported in the two ways described below.
