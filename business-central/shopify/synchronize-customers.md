...
|Field|Description|
|------|-----------|
|**Export customers to Shopify**|Select this if you plan to export all customers with a valid email address from [!INCLUDE[prod_short](../includes/prod_short.md)] to Shopify in bulk. You can do it either manually, using the **Sync Customers** action, or automatically, using a job queue for recurring updates.<br> When exporting customers with addresses that include provinces/counties, make sure that **ISO Code** is filled in for countries/regions.|
