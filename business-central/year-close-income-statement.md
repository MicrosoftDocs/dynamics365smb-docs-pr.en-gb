...
description: At year closing, you must run the Close Income Statement batch job to close the accounting periods that make up the financial year.
author: jswymer
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
ms.locfileid: "8514680"
---
# <a name="closing-income-statement-accounts"></a>Closing Income Statement Accounts
When a financial year is over, you must close the periods that comprise it. To do this, you run the **Close Income Statement** batch job. This job transfers the year's result to an account in the balance sheet and closes the income statement accounts. You do this by creating lines in a journal, which you then can post.

## <a name="to-run-the-close-income-statement-batch-job"></a>To run the Close Income Statement batch job
1. Close the financial year. The financial year must closed before the batch job can be run. For more information, see [Close Accounting Periods](year-close-account-periods.md).
...
The date on the lines that the batch job inserts in the journal is always a closing date for the financial year. The closing date is a fictitious date between the last day of the old financial year and the first day of the new year. The advantage of posting on a closing date is that you maintain the correct balances for the ordinary dates of the financial year.

The **Close Income Statement** batch job can be used several times. You can post in a previous financial year, even after the income statement accounts have been closed, if you run the batch job again.
