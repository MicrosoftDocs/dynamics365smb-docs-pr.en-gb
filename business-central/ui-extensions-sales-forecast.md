---
title: Using the Sales and Stock Forecast Extension to Manage Stock | Microsoft Docs
...
ms.locfileid: "6434841"
---
# <a name="the-sales-and-inventory-forecast-extension"></a>The Sales and Stock Forecast Extension
Stock management is a trade-off between customer service and managing your cost. On one hand, a low stock requires less working capital, but, on the other hand, stock-outs potentially lead to missed sales. The Sales and Stock Forecast extension predicts potential sales using historical data and gives a clear overview of expected stock-outs. Based on the forecast, the extension helps create replenishment requests to your suppliers and saves you time.  
...
The extension uses Azure AI to predict future sales based on your sales history to help you avoid stock shortage. For example, when you choose an item on the **Items** page, the chart in the **Item Forecast** pane shows the estimated sales of this item in the coming period. This way you can see if you are likely to run out of stock of the item soon.  

You can also use the extension to suggest when to stock up on stock. For example, if you create a purchase order for Fabrikam because you want to buy their new desk chair, the Sales and stock Forecast extension will suggest that you also restock on the LONDON swivel chair that you usually buy from this supplier. This is because the extension forecasts that you will run out of stock of the LONDON swivel chair in the coming two months, so you might want to order more chairs already now.  
...
> [!NOTE]  
>   You can also use your own predictive web service instead of ours. For more information, see [Create and use your own predictive web service for sales and stock forecasts](#AnchorText). 
...
- The posting date is between the date that is calculated based on the values in the **Historical Periods** and **Period Type** fields on the **Sales and Stock Forecast Setup** page and the work date.

Before using the web service [!INCLUDE[prod_short](includes/prod_short.md)] compresses transactions by **Item No.** and **Posting Date** based on the value in the **Period Type** field in the **Sales and Stock Forecast Setup** page.

## <a name="create-and-use-your-own-predictive-web-service-for-sales-and-inventory-forecasts"></a><a name="AnchorText"> </a>Create and use your own predictive web service for sales and stock forecasts
...
6. Choose the ![Lightbulb that opens the Tell Me feature.](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **Sales and Stock Forecast Setup**, and then choose the related link.  
